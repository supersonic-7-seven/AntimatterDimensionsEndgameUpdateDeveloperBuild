...
export const dilationUpgrades = {
  ...
  galaxyThreshold: rebuyable({
    ...
    description: () =>
      ((Perk.bypassTGReset.isBought && !player.disablePostReality) && (!Pelle.isDoomed || PellePerkUpgrade.perkTGR.isBought)
  ...
  flatDilationMult: {
    ...
    effect: () => Decimal.pow(1e9, Decimal.min(Decimal.pow(Decimal.max(player.eternityPoints.add(1).log10().sub(1500), 0).div(2500), 1.2), 1).times(player.eternityPoints.add(1).log10().sub(4000).max(1).log10())),
