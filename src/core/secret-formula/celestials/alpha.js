export const alphaUnlocks = {
  ...
  breakInfinity: {
    id: 5,
    requirement: 6,
    nerfDescription: () => `Break Infinty Upgrades are ${formatX(1000)} more expensive, post-Break Tickspeed/Dimension cost scalings start at ${formatX(20)}, IPow Conversion Rate /${format(DC.D7.div(Decimal.log10(Decimal.log10(Currency.infinityPoints.value.add(1)).add(1)).pow(2)).toNumber(), 2, 2)} (based on IP)`,
    ...
    effects: {
      nerfA: 1000,
      nerfB: 20,
      nerfC: () => DC.D7.div(Decimal.log10(Decimal.log10(Currency.infinityPoints.value.add(1)).add(1)).pow(2)).toNumber(),
      buffA: 0.15,
      buffB: 0.25
    }
  },
