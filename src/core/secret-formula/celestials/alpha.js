export const alphaUnlocks = {
  ...
  infinityDimensions: {
    id: 10,
    requirement: 11,
    nerfDescription: () => `Infinity Point gain is raised ${formatPow(Math.clamp(1 - Decimal.log10(Currency.antimatter.value.add(1)).sub(75000).div(100000).toNumber(), 0, 1), 2, 3)} (only applies first Eternity)`,
    buffDescription: () => `The ${formatInt(8)}th Infinity Dimension is powered ${formatInt(100)}`,
    effects: {
      nerf: () => Math.clamp(1 - Decimal.log10(Currency.antimatter.value.add(1)).sub(75000).div(100000).toNumber(), 0, 1),
      buff: 100
    }
  },
  eternity: {
