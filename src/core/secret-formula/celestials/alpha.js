export const alphaUnlocks = {
  ...
  timeDimension8: {
    id: 26,
    requirement: 27,
    nerfDescription: () => `Eternity Point gain is raised ${formatPow(Math.clamp(1 - Decimal.log10(player.records.thisEternity.maxIP.add(1)).sub(1.5e6).div(1e7).min(0).pow(0.5).toNumber(), 0, 1), 2, 3)}`,
    buffDescription: () => `The ${formatInt(8)}th Time Dimension is powered ${formatInt(1000)}`,
    effects: {
      nerf: () => Math.clamp(1 - Decimal.log10(player.records.thisEternity.maxIP.add(1)).sub(1.5e6).div(1e7).min(0).pow(0.5).toNumber(), 0, 1),
      buff: 1000
    }
  },
  reality: {
