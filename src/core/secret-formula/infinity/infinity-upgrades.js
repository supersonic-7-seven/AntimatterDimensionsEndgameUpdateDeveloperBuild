...
export const infinityUpgrades = {
  ...
  ipMult: {
    ...
    checkRequirement: () => Achievement(41).isUnlocked,
    costCap: () => Alpha.isRunning ? Decimal.pow10(AlphaUnlocks.infinityChallenges.effects.nerf.effectValue).times(10) : ((BreakEternityUpgrade.doubleIPUncap.isBought && !player.disablePostReality) ? DC.BEMAX : DC.E6E6),
    ...
    effect: () => (player.IPMultPurchases.gte(3300000) && (!BreakEternityUpgrade.doubleIPUncap.isBought || player.disablePostReality) ? DC.E1E6 : DC.D2.pow(player.IPMultPurchases)),
    cap: () => {
      const normcap = (BreakEternityUpgrade.doubleIPUncap.isBought && !player.disablePostReality) ? DC.BEMAX : DC.E1E6;
      return Alpha.isRunning ? Decimal.pow(2, AlphaUnlocks.infinityChallenges.effects.nerf.effectValue) : (Effarig.eternityCap ?? normcap);
    },
    formatEffect: value => formatX(value, 2, 2),
  }
};
