export const glyphSacrifice = {
  "power": {
    ...
    effect: added => {
      ...
      const base = Decimal.log10(capped.add(1)) / 100;
  ...
  "infinity": {
    ...
    effect: added => {
      ...
      const capped = Decimal.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return 1 + Decimal.log10(Decimal.pow(capped, 0.2).div(100).add(1));
    },
  ...
  "time": {
    ...
    effect: added => {
      ...
      const capped = Decimal.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      return Decimal.pow(Decimal.pow(capped, 0.2).div(100).add(1), 2).toNumber();
    },
  ...
  "dilation": {
    ...
    effect: added => {
      ...
      const capped = Decimal.clampMax(sac, GlyphSacrificeHandler.maxSacrificeForEffects);
      const exponent = 0.32 * Math.pow(Decimal.log10(capped.add(1)) / 100, 0.1);
      return Decimal.pow(Decimal.clampMin(capped, 1), exponent).toNumber();
    },
