...
export const dualityUpgrades = [
  ...
  },
  {
    ...
    hasFailed: () => Glyphs.active.filter(x => x.type !== "cursed").length > 0 || player.requirementChecks.reality.maxGlyphs > -15,
    checkRequirement: () => Glyphs.active.filter(x => x.type !== "cursed").length === 0 &&
