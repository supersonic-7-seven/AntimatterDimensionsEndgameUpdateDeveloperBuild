...
export const MachineHandler = {
  ...
  get baseIMCap() {
    ...
    return (Decimal.pow(Decimal.clampMin(new Decimal(this.uncappedRM.log10()).sub(1000), 0), 2).times(
      ...
      new Decimal(Decimal.log10(this.uncappedRM.log10())).div(7.5)))).pow(Effects.product(EndgameMastery(144)));
  },
  ...
  estimateIMTimer(cost) {
    ...
    return Decimal.max(0, Decimal.log2(imCap.div(imCap.sub(cost))).sub(Decimal.log2(imCap.div(imCap.sub(currentIM))))).times(
      this.scaleTimeForIM);
  }
};
