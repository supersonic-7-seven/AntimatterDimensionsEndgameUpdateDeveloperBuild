export function effectiveBaseGalaxies() {
  ...
  freeGalaxies = freeGalaxies.times(Decimal.max(0, Replicanti.amount.log10().div(1e6)).times(AlchemyResource.alternation.effectValue).add(1));
...
export function getTickSpeedMultiplier() {
  ...
  if (Pelle.isDoomed && EndgameMilestone.remnantGalaxy.isReached) galaxies = galaxies.times(Decimal.pow(1 + Math.log10(Currency.remnants.value + 1), 0.5));
...
export const FreeTickspeed = {
  ...
  fromShards(shards) {
    ...
    const logShards = Decimal.min(1, shards).ln().toNumber();
